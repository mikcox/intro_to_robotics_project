...
cv::Mat greenify(cv::Mat mat){
	 assert(mat.type() == CV_8UC3);

	 cv::Mat greenonly;
    cv::inRange(mat, cv::Scalar(0, gl, 0), cv::Scalar(0, 255, 0), greenonly);
	 return greenonly;
  }
...
public:
  ...
  void imageCb(const sensor_msgs::ImageConstPtr& msg)
  {
	...
	//greenify:
	 cv::Mat greenOnly;
    cv::inRange(cv_ptr->image, cv::Scalar(0, 209, 0), cv::Scalar(255, 255, 255), greenOnly);

    /* Add any OpenCV processing here */
 /* Gray scale image */
    cv::Mat filtered_image = greenOnly;
   // cv::cvtColor(greenOnly,filtered_image,CV_BGR2GRAY,4);
    ...
    */
    ...
    for (; i >=0; i=hierarchy[i][0]) {
      ...
      for (int j=0; j<4; j++) {
		...
		int centerx = (rect_points[0].x+rect_points[1].x+rect_points[2].x+rect_points[3].x)/4;
		int centery = (rect_points[0].y+rect_points[1].y+rect_points[2].y+rect_points[3].y)/4;
		//std::cout<<"Corners: "<<rect_points[0]<<", "<<rect_points[1]<<", "<<rect_points[2]<<", "<<rect_points[3]<<". "<<std::endl;
	...
	//show greenified image 
    cv::namedWindow("Greenified Image");
    cv::imshow("Greenified Image",greenOnly);
	 cv::createTrackbar("gl", "Greenified Image", &gl, 255);
